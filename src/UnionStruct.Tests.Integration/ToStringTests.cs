...
public sealed class ToStringTests
{
	...
	public void ToStringReturnsCorrectResult()
	{
		...
		Assert.Equal("8-bit { Value = 1 }", CompressedIndex.Unsigned8(1).ToString());
		Assert.Equal("16-bit { Value = 2 }", CompressedIndex.Unsigned16(2).ToString());
		Assert.Equal("32-bit { Value = 3 }", CompressedIndex.Unsigned32(3).ToString());
		...
		Assert.Equal("Nested { Value = Empty }", RootUnion.NestedCase(NestedUnion.Empty()).ToString());
		Assert.Equal("Nested { Value = Node { Value = 1 } }", RootUnion.NestedCase(NestedUnion.Node(1)).ToString());
	}
}
